...
const Digest = () => {
    ...
    return (
        <div className="min-h-screen relative">
            ...
            {/* Header Navigation */}
            <div className="backdrop-blur-3xl top-0 z-40">
                    ...
                    {selectedTrack && (
                        <div className="mt-8 pt-8 border-t border-gray-100">
                            ...
                            <AudioPlayer
                                ...
                                title={`Episode ${deliveries.findIndex(d => d.id === selectedTrack) + 1}`}
                                subtitle={new Date(deliveries.find(d => d.id === selectedTrack)?.createdAt || '').toLocaleDateString()}
                                variant="full"
                                autoPlay
                                onEnded={() => {
                                    // Auto-play next episode if available
                                    const currentIndex = deliveries.findIndex(d => d.id === selectedTrack)
                                    if (currentIndex < deliveries.length - 1) {
                                        setSelectedTrack(deliveries[currentIndex + 1].id)
                                    }
                                }}
                            />
                        </div>
